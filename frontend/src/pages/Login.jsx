...
import { Shield, BookOpen, GraduationCap, Lock, Mail, BarChart3, Users, X, BadgeCheck, ArrowLeft, KeyRound, RotateCw } from 'lucide-react';

export default function Login() {
  const { user, setUser, loading } = useAuth();
  const [activeRole, setActiveRole] = useState('hod'); // 'hod', 'faculty', 'student'
  
  const [step, setStep] = useState(1); // 1 = credentials form, 2 = OTP email code verification
  ...
  const handleResendCode = async () => {
    ...
    setResending(true);
    try {
      const { data } = await api.post('/auth/resend-code', { email: pendingEmail, type: 'login' });
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setAuthLoading(true);
    try {
      const { data } = await api.post('/auth/verify-code', { email: pendingEmail, otp });
      localStorage.setItem('token', data.token);
      localStorage.setItem('user', JSON.stringify(data.user));
      setUser(data.user);
            ...
            <div className="glass p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl relative bg-white/95 dark:bg-slate-900/95">
              ...
              <div className="text-center mb-6">
                ...
                <h1 className="text-xl font-bold tracking-tight text-slate-800 dark:text-white transition-all duration-350">
                  {step === 2 ? 'Verify Your Email Code' : currentRole.title}
                </h1>
                <p className="text-xs text-slate-550 dark:text-slate-400 mt-1 transition-all duration-350">
                  {step === 2 ? `Enter 6-digit code sent to ${pendingEmail}` : currentRole.subtitle}
                </p>
              </div>
              ...
              {step === 1 ? (
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input 
                          ...
                          required 
                          autoComplete="off"
                          placeholder={activeRole === 'student' ? 'Email or USN (e.g. 4PM22CS001)' : 'name@example.com'}
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-4">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-450 dark:text-slate-500 mt-1 text-center">
                      Code sent to <span className="font-bold text-slate-700 dark:text-slate-300">{pendingEmail}</span>
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-800">
                    <button
                      ...
                      className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline disabled:text-slate-400 dark:disabled:text-slate-600 disabled:no-underline flex items-center gap-1.5 bg-transparent border-0 cursor-pointer"
                    >
